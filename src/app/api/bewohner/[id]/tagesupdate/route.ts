...
export async function GET(_req: NextRequest, { params }: Params) {
  try {
    ...
    logger.error("GET /api/bewohner/[id]/tagesupdate", { error: err instanceof Error ? err.message : String(err) });
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    logger.error("POST /api/bewohner/[id]/tagesupdate", { error: err instanceof Error ? err.message : String(err) });
    return NextResponse.json({ error: "Interner Fehler" }, { status: 500 });
  }
}
